...
export default function MatchPostPage() {
  ...
  const openReportModal = (targetId: number) => {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      setIsPostModalOpen(false);
      setPostContent('');
      router.push(`/forums/match/threads/${threadId}/posts/${body.id}`);
    } catch (err: any) {
  ...
  const loadPost = useCallback(async () => {
    ...
    try {
      ...
      if(data.visibility === false) {
        router.push(`/forums/match/threads/${params.threadId}`);
      }
    } catch (err: any) {
  ...
  const handleReportAction = async () => {
    ...
    try {
      const response = await fetch('/api/users/reports', {
        ...
        body: JSON.stringify({ reason: trimmed, targetId: reportingTargetId, targetType: 'POST' }),
      });
      const body = await response.json();
      if (!response.ok) {
  ...
  const handleUpdatePost = async () => {
    ...
    try {
      ...
      setIsEditModalOpen(false);
      setEditContent('');
      router.push(`/forums/match/threads/${post.thread?.id ?? threadId}/posts/${body.id}`);
    } catch (err: any) {
  ...
  const handleDeletePost = async () => {
    ...
    openConfirm('Delete post?', 'This will hide it for everyone.', async () => {
      ...
      try {
        ...
        router.push(`/forums/match/threads/${post.thread?.id ?? threadId}`);
      } catch (err: any) {
      ...
      <header className="relative overflow-hidden border-b border-gray-200/50 dark:border-gray-800 bg-white/80 dark:bg-[#1f2937]/80 backdrop-blur-xl">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-12">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500 dark:text-gray-400">
            ...
            <Link
              href={`/forums/match/threads/${post.thread?.id ?? threadId}`}
              className="rounded-full border border-sky-200/50 dark:border-sky-900/30 bg-sky-50 dark:bg-sky-900/20 px-4 py-1.5 font-bold text-sky-700 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/40 transition"
            >
              {threadTitle}
            </Link>
          </div>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={() =>
                previousVersionId &&
                router.push(`/forums/match/threads/${post.thread?.id ?? threadId}/posts/${previousVersionId}`)
              }
            ...
            <button
              ...
              onClick={() =>
                nextVersionId &&
                router.push(`/forums/match/threads/${post.thread?.id ?? threadId}/posts/${nextVersionId}`)
              }
            ...
            <span className="flex items-center gap-2">
              ...
              {post.author?.username ? (
                <button
                  ...
                  className="text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 font-bold hover:underline transition-colors"
                >
                  @{post.author.username}
                </button>
              ) : (
        ...
        <section className={`rounded-3xl relative overflow-hidden p-8 shadow-sm backdrop-blur-sm transition-all duration-300 ${
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            ...
            {(post.previousVersionId || post.nextVersionId) && (
              <button
                ...
                className="text-xs font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300 transition"
              >
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-gray-100 dark:border-gray-800/50 pt-6">
            ...
            <button
              ...
              disabled={!hasToken}
              className="inline-flex items-center gap-2 rounded-full border border-sky-200/50 dark:border-sky-900/50 bg-sky-50/50 dark:bg-sky-900/20 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-sky-600 dark:text-sky-400 transition hover:bg-sky-100 dark:hover:bg-sky-900/40 disabled:cursor-not-allowed disabled:opacity-50 shadow-sm"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
          ...
          {replies.length === 0 ? (
            ...
            <div className="space-y-6">
              {displayedReplies.map((reply) => (
                <article key={reply.id} className="rounded-2xl border border-gray-200/50 dark:border-gray-700/50 bg-white dark:bg-[#1f2937] p-6 shadow-sm transition-all hover:shadow-md relative before:absolute before:left-0 before:top-4 before:bottom-4 before:w-1 before:rounded-r-full before:bg-sky-500/50 overflow-hidden">
                  ...
                  <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-t border-gray-100 dark:border-gray-800/50 pt-4">
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={!hasToken}
                        className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-rose-400 hover:text-rose-600 dark:hover:text-rose-300 transition-colors pointer-events-auto"
                      >
                      ...
                      <button
                        ...
                        disabled={!hasToken}
                        className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-400 hover:text-sky-600 dark:hover:text-sky-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5">
                          <path fillRule="evenodd" d="M7.707 3.293a1 1 0 010 1.414L5.414 7H11a7 7 0 017 7v2a1 1 0 11-2 0v-2a5 5 0 00-5-5H5.414l2.293 2.293a1 1 0 11-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
            ...
            <div className="mt-6 space-y-6">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30 px-4 py-3 text-sm text-gray-700 dark:text-gray-200 placeholder:text-gray-400 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 focus:outline-none transition-all"
                />
              </div>
              {postCreateError && (
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                ...
                <button
                  ...
                  className="rounded-full bg-sky-600 dark:bg-sky-500 px-8 py-2.5 text-xs font-bold uppercase tracking-widest text-white transition hover:bg-sky-700 dark:hover:bg-sky-400 disabled:cursor-not-allowed disabled:bg-gray-200 dark:disabled:bg-gray-700 shadow-lg shadow-sky-600/20"
                >
          ...
          <div className="w-full max-w-md rounded-3xl bg-white dark:bg-[#1f2937] p-8 shadow-2xl animate-in fade-in zoom-in duration-200 border border-gray-100 dark:border-gray-800">
            ...
            <div className="mt-6 space-y-6">
              <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed font-medium">
                Please specify why you are reporting this post. Our moderators will review it shortly.
              </p>
