...
export default function MatchThreadPage() {
  ...
  const [reportingTargetId, setReportingTargetId] = useState<number | null>(null);
  const [reportingTargetType, setReportingTargetType] = useState<'THREAD' | 'POST'>('THREAD');

  const openReportModal = (targetId: number, targetType: 'THREAD' | 'POST') => {
    setReportingTargetId(targetId);
    setReportingTargetType(targetType);
    setReportReason('');
    setReportError('');
    setReportStatus('');
  ...
  const handleReportAction = async () => {
    if (!hasToken || reportSubmitting || !reportingTargetId) return;
    setReportError('');
    setReportStatus('');
    ...
    try {
      ...
      setTimeout(() => setIsReportModalOpen(false), 1500);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gray-50/50 dark:bg-[#0B1121] selection:bg-sky-500/30">
      <header className="relative overflow-hidden border-b border-gray-200/50 dark:border-gray-800 bg-white/80 dark:bg-[#1f2937]/80 backdrop-blur-xl">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-6 px-6 py-12">
          <div className="inline-flex w-fit items-center gap-2 rounded-full border border-sky-200/50 bg-sky-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-sky-600 dark:border-sky-700/30 dark:bg-sky-900/20 dark:text-sky-400 shadow-sm">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-3xl">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-4 text-sm font-medium text-gray-500 dark:text-gray-400">
                <p className="flex items-center gap-2 rounded-full border border-sky-200/50 bg-sky-50 dark:bg-sky-900/30 px-3 py-1 font-bold text-sky-700 dark:text-sky-400">
                  ⚽ Match:{' '}
                  {thread.matchDiscussionFor?.homeTeam?.name && thread.matchDiscussionFor?.awayTeam?.name
                    ? `${thread.matchDiscussionFor.homeTeam.name} vs ${thread.matchDiscussionFor.awayTeam.name}`
                    : 'Match TBD'}
                </p>
                  ...
                  {thread.author && (
                    <>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 transition-colors"
                      >
            ...
            <div className="flex flex-col items-stretch sm:items-end gap-2">
                <button
                  type="button"
                  onClick={() => openReportModal(thread.id, 'THREAD')}
                  disabled={!hasToken}
                  className="inline-flex items-center justify-center gap-2 rounded-full border border-rose-200/50 dark:border-rose-900/50 bg-rose-50/50 dark:bg-rose-900/20 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400 transition hover:bg-rose-100 dark:hover:bg-rose-900/40 disabled:cursor-not-allowed disabled:opacity-50 shadow-sm"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
              ...
              {(isOwner || isAdmin) && (
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <button
                    ...
                    className="rounded-full border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 px-4 py-2 text-sm font-semibold text-red-700 dark:text-red-400 transition hover:bg-red-100 dark:hover:bg-red-900/40 disabled:cursor-not-allowed disabled:opacity-60 shadow-sm"
                  >
      ...
      <main className="mx-auto max-w-6xl px-6 py-12 space-y-16">
        <section className="space-y-6">
          ...
          {thread?.mainPost && thread.mainPost.visibility !== false ? (
            <div className="relative overflow-hidden rounded-3xl border border-sky-200/50 dark:border-sky-900/40 bg-gradient-to-br from-sky-50/50 to-white dark:from-sky-900/10 dark:to-[#1f2937]/50 p-8 shadow-sm backdrop-blur-sm transition-all duration-300 hover:shadow-lg group">
              <Link
                href={`/forums/match/threads/${thread.id}/posts/${thread.mainPost.id}`}
                className="absolute inset-0 z-0"
              />
              <div className="pointer-events-none absolute top-0 right-0 rounded-bl-3xl bg-gradient-to-r from-sky-500 to-sky-600 px-5 py-2 text-[10px] font-bold uppercase tracking-widest text-white shadow-sm z-10">
                Original Post
              </div>
              ...
              <div className="relative z-10 mt-3 flex justify-start pointer-events-auto">
                <button
                  ...
                  className="text-[10px] font-bold uppercase tracking-wider text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 transition"
                >
                  {isTranslating[thread.mainPost.id] ? "Translating..." : translatedContent[thread.mainPost.id] ? "Show original" : "Translate"}
                </button>
              </div>
              <div className="relative z-10 mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-sky-100 dark:border-sky-900/30 pt-4 pointer-events-none">
                  ...
                  {thread.mainPost.author && (
                    <>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 transition-colors pointer-events-auto"
                      >
        ...
        <section className="space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex items-center gap-3 w-full max-w-md">
              <div className="flex-1 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1f2937] px-4 py-2 shadow-sm">
                <input
                  type="text"
                  value={postsQuery}
              ...
              <button
                ...
                disabled={!hasToken}
                className="whitespace-nowrap rounded-full bg-sky-600 px-6 py-2 text-sm font-bold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-gray-200 dark:disabled:bg-gray-700 shadow-lg shadow-sky-600/20"
              >
                {hasToken ? 'Add post' : 'Sign in'}
              </button>
            </div>
          </div>
          {deletePostError && (
                ...
                const renderPost = (node: PostRecord, depth: number) => {
                  ...
                  return (
                    <div key={node.id} style={{ marginLeft: depth * 24 }}>
                      <Link
                        href={`/forums/match/threads/${thread.id}/posts/${node.id}`}
                        className="block"
                      >
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex items-center gap-3 w-full max-w-md">
              ...
              <button
                ...
                disabled={!hasToken}
                className="whitespace-nowrap rounded-full border border-blue-200 bg-white dark:bg-[#1f2937] px-4 py-2 text-sm font-semibold text-blue-700 dark:text-blue-400 transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:border-gray-200 disabled:text-gray-400 shadow-sm"
              >
                {hasToken ? 'Create poll' : 'Sign in'}
              </button>
            </div>
          </div>
          {deletePollError && (
          ...
          {pollsLoading ? (
            ...
            <div className="space-y-4">
              {pollsOnPage.map((poll) => (
                <Link
                  key={poll.id}
                  href={`/forums/match/threads/${thread.id}/polls/${poll.id}`}
                  className="block"
                >
                  <article className="rounded-3xl border border-gray-200/50 dark:border-gray-800 bg-white/70 dark:bg-[#1f2937]/50 p-6 shadow-sm backdrop-blur-sm transition-all duration-300 hover:shadow-md group">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">{poll.pollDescription}</h3>
                      ...
                      {poll.author && (
                        <>
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-sky-600 hover:text-sky-500 dark:text-sky-400 dark:hover:text-sky-300 transition-colors pointer-events-auto font-bold uppercase"
                          >
      ...
      {isReportModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white dark:bg-[#1f2937] p-8 shadow-2xl animate-in fade-in zoom-in duration-200 border border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                    <path fillRule="evenodd" d="M3 6a3 3 0 013-3h10a1 1 0 01.8 1.6L14.25 12l2.55 7.4a1 1 0 01-.8 1.6H6a1 1 0 01-1-1V6z" clipRule="evenodd" />
                  </svg>
                </div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                  Report {reportingTargetType === 'THREAD' ? 'Thread' : 'Post'}
                </h2>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 space-y-6">
              <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed font-medium">
                Please specify why you are reporting this {reportingTargetType.toLowerCase()}. Our moderators will review it shortly.
              </p>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Inappropriate content, spam, harassment..."
                  rows={4}
                  className="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/30 px-4 py-3 text-sm text-gray-700 dark:text-gray-200 placeholder:text-gray-400 focus:border-rose-500 focus:ring-1 focus:ring-rose-500 focus:outline-none transition-all resize-none"
                />
              </div>

              {reportError && (
                <div className="rounded-xl border border-rose-200 dark:border-rose-900/30 bg-rose-50/50 dark:bg-rose-900/20 p-3 flex items-center gap-3">
                  <span className="text-rose-600 dark:text-rose-400 font-bold text-xs">{reportError}</span>
                </div>
              )}

              {reportStatus && (
                <div className="rounded-xl border border-emerald-200 dark:border-emerald-900/30 bg-emerald-50/50 dark:bg-emerald-900/20 p-3 flex items-center gap-3">
                  <span className="text-emerald-600 dark:text-emerald-400 font-bold text-xs">{reportStatus}</span>
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                <button
                  type="button"
                  onClick={closeReportModal}
                  className="rounded-full px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-full bg-rose-600 px-8 py-2.5 text-xs font-bold uppercase tracking-widest text-white transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:bg-gray-200 dark:disabled:bg-gray-700 shadow-lg shadow-rose-600/20"
                >
