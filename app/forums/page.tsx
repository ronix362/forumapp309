...
type ThreadItem = {
  ...
  views: number;
  lastActive: string;
  createdAtRaw: string; // <-- Added to allow accurate date sorting
  tag?: string;
  pinned?: boolean;
  teamId?: number; 
};
...
const mapThread = (thread: any): ThreadItem => {
  ...
  return {
    ...
    createdAtRaw: thread.createdAt, // <-- Raw date for accurate sorting
    tag,
...
function ThreadCard({ thread, href }: { thread: ThreadItem; href: string }) {
  // FIX: Using group/thread to isolate the hover states from the parent CategoryCard group
  return (
    <Link href={href} className="group/thread block focus:outline-none">
      <article className="rounded-3xl border border-white/60 dark:border-gray-700/50 bg-white/70 dark:bg-[#374151]/50 p-5 shadow-sm backdrop-blur-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg hover:border-blue-200 dark:hover:border-blue-800/50">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            {/* FIX: Changed group-hover to group-hover/thread */}
            <h3 className="line-clamp-2 text-lg font-bold text-gray-900 dark:text-white transition-colors group-hover/thread:text-blue-600 dark:group-hover/thread:text-blue-400">
              {thread.title}
            </h3>
            {/* FIX: Changed group-hover to group-hover/thread */}
            <p className="mt-2 line-clamp-2 text-sm text-gray-600 dark:text-gray-300 group-hover/thread:text-gray-900 dark:group-hover/thread:text-gray-200 transition-colors">
              {thread.excerpt}
            </p>
          </div>
          {/* FIX: Changed group-hover to group-hover/thread */}
          <div className="hidden h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white dark:bg-[#1f2937] text-sm font-bold text-blue-600 dark:text-blue-400 shadow-sm border border-gray-100 dark:border-gray-800 sm:flex transition-transform duration-300 group-hover/thread:scale-110 group-hover/thread:shadow-blue-200 dark:group-hover/thread:shadow-blue-900/20">
            {thread.replies}
...
export default function ForumsPage() {
  ...
  const categories = useMemo<CategoryWithThreads[]>(() => {
    ...
    return categoryStyles.map((category) => {
      const categoryThreads = [...(threadMap[category.type] || [])];

      // Sort based on current sortMode state
      if (sortMode === 'popular') {
        categoryThreads.sort((a, b) => b.replies - a.replies);
      } else {
        // 'recent' mode (sort by date descending)
        categoryThreads.sort((a, b) => new Date(b.createdAtRaw).getTime() - new Date(a.createdAtRaw).getTime());
      }

      return {
        ...category,
        totalCount: categoryThreads.length,
        threads: categoryThreads.slice(0, 3), // Grab top 3 after sorting
      };
    });
  }, [generalThreads, matchThreads, teamThreads, sortMode]);
    ...
    <div className="min-h-screen bg-gray-50/50 dark:bg-[#0B1121] selection:bg-blue-500/30">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12 lg:py-16">
        <div className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          {/* Filtering Toggle */}
          <div className="flex items-center rounded-full bg-gray-200/50 p-1 dark:bg-gray-800/50">
            <button
              onClick={() => setSortMode('recent')}
              className={`rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${
                sortMode === 'recent'
                  ? 'bg-white text-blue-600 shadow-sm dark:bg-[#374151] dark:text-blue-400'
