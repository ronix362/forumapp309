...
export default function GeneralForumsPage() {
  ...
  const loadThreads = useCallback(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch general threads');
      }
      const data = (await response.json()) as ThreadRecord[];
      setThreads(Array.isArray(data) ? data : []);
      setPage(1);
    } catch (err: any) {
      setError(err.message || 'Failed to load general threads');
      setThreads([]);
    } finally {
      setIsLoading(false);
    }
  ...
  }, [authorQuery, filterTags, loadThreads, teamQuery, titleQuery]);
  ...
  const handleCreateThread = async () => {
    ...
    try {
      const response = await fetch('/api/threads', {
        ...
        body: JSON.stringify({ title: trimmedTitle, type: 'GENERAL' }),
      });
      ...
      <header className="relative overflow-hidden border-b border-gray-200/50 dark:border-gray-800 bg-white/80 dark:bg-[#1f2937]/80 backdrop-blur-xl">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-8 px-6 py-12 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-amber-200/50 bg-amber-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-amber-600 dark:border-amber-700/30 dark:bg-amber-900/20 dark:text-amber-400 shadow-sm">
            ...
            <p className="mt-4 text-base font-medium text-gray-600 dark:text-gray-300">
              Browse tactical analysis, transfers, and the big football conversations.
            </p>
          </div>
          ...
          <div className="space-y-4">
            {pageThreads.length === 0 ? (
              ...
              pageThreads.map((thread) => (
                <Link
                  key={thread.id}
                  href={`/forums/general/threads/${thread.id}`}
                  ...
                  <article className="flex flex-col gap-4 rounded-3xl border border-gray-200/70 dark:border-gray-800 bg-white/70 dark:bg-[#1f2937]/50 p-6 shadow-sm backdrop-blur-sm transition-all duration-300 ease-out group-hover:-translate-y-1 group-hover:shadow-lg group-hover:border-amber-200 dark:group-hover:border-amber-800/50">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center rounded-full border border-amber-200/50 bg-amber-50 dark:bg-amber-900/30 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 shadow-sm">
                          General
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-4 text-xs font-medium text-gray-500 dark:text-gray-400">
                       ...
                       <div className="flex items-center gap-3">
                         ...
                         <span className="opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-1 text-amber-600 dark:text-amber-400 font-bold">
                           View Thread →
                         </span>
                       </div>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-[#1f2937] p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Create General Thread</h2>
              <button
                type="button"
                onClick={closeModal}
